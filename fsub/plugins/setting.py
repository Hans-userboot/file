...
from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton, Message # <- Ditambahkan Message
from fsub import *
from fsub.database.data import * # Pastikan semua fungsi database terimport
...
async def protect_settings(client, callback_query):
    ...
    # Konversi status ke string text untuk tampilan tombol yang dinamis
    button = InlineKeyboardButton(
        "❌ Matikan lindungi media" if str(status) == "True" else "✅ Aktifkan lindungi media",
...
async def toggle_protect(client, callback_query):
    ...
    await add_setting(user_id, disable, not status if isinstance(status, bool) else status != "True")
...
async def disable_settings(client, callback_query):
    ...
    button = InlineKeyboardButton(
        "❌ Matikan button media" if str(status) == "True" else "✅ Aktifkan button media",
...
async def toggle_disable(client, callback_query):
    ...
    # Toggle status True/False
    await add_setting(user_id, not status if isinstance(status, bool) else status != "True", anti)
...
async def caption_button(client, callback_query):
    try:
        # Perbaikan teks instruksi agar sesuai peruntukan custom caption
        isi: Message = await client.ask(
            chat_id=callback_query.from_user.id,
            text="✍️ **Silakan masukkan Custom Caption baru Anda:**\n\nKetik `/batal` untuk membatalkan proses.",
        ...
        await add_caption(bot_id, isi.text)
        await isi.reply("✅ Custom caption berhasil diupdate!")
    except Exception as e:
        await callback_query.message.reply(f"❌ Terjadi error: {e}")
    ...
    try:
        ...
        try:
            ...
            await channel_input.reply(
                ...
                "Pastikan:\n"
                "1. ID Channel benar\n"
                "2. Bot sudah menjadi admin di channel tersebut\n"
                "3. Bot punya izin untuk mengundang pengguna via link"
            )
            return
...
async def list_fsub_channels(client, callback_query):
    try:
        ...
        if not channels:
            await callback_query.edit_message_text(
                "❌ Tidak ada channel FSub yang terdaftar",
        ...
        for channel in channels:
            try:
                ...
                buttons.append([
                    InlineKeyboardButton(f"🗑 {chat.title}", callback_data=f"confirm_hapus_{chat.id}")
                ])
            except:
                buttons.append([
                    InlineKeyboardButton(f"🗑 Error ID: {channel}", callback_data=f"confirm_hapus_{channel}")
                ])
    ...
    try:
        ...
        if not await cek_fsub(channel_id):
            await callback_query.edit_message_text(
                ...
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Kembali", callback_data="fsub_hapus")]])
            )
            return
            
        await del_fsub(channel_id)
        await callback_query.edit_message_text(
            f"✅ **Channel Berhasil Dihapus**\n\nID: `{channel_id}` kini telah dihapus dari daftar FSub.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Kembali ke Daftar", callback_data="fsub_hapus")]])
        )
    except Exception as e:
        await callback_query.edit_message_text(
